...
use scrollz::core::rmw::CodecParams;
use scrollz::core::wsession::WriteSession;
use scrollz::core::DEFAULT_CHUNK_SIZE;
...
fn run_on_store(
    ...
    // 隔离本段的块压缩（重压）计数。
    scrollz::core::rmw::reset_block_compress_count();
    ...
    RunResult {
        ...
        logical_bytes,
        physical_bytes,
        recompress: scrollz::core::rmw::block_compress_count(),
        blocks: block_count(),
    }
}
...
fn shadow_block_count(archive: &Path) -> u64 {
    scrollz::archive::ArchiveReader::open(archive)
        .map(|r| r.chunk_count())
        .unwrap_or(0)
}
