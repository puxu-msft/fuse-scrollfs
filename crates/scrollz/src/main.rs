...
use scrollz::core::blockcache::DEFAULT_CACHE_BYTES;
...
struct EnableArgs {
    #[command(subcommand)]
    action: Option<scrollz::enable::EnableAction>,
}
...
fn run_mount_managed(args: MountManagedArgs) -> std::io::Result<()> {
    ...
    scrollz::enable::guard_underlay_or_exit(&paths, &name)?;
    // 评审 H1：unescape 有损（裸 `-`→`/`），非法 %i 会被扭曲。出错时附原始实例名便于反查。
    let spec = scrollz::enable::systemd::resolve_managed_spec(&paths, &name).map_err(|e| {
        std::io::Error::new(
            e.kind(),
            format!("{e}（systemd 实例 %i={:?} → 解码名 {name:?}）", args.name),
        )
...
fn mount_args_from_spec(spec: &scrollz::enable::daemon::MountSpec) -> MountArgs {
    use scrollz::enable::model::Backend as MBackend;
...
struct SealArgs {
    ...
    #[arg(long, default_value_t = scrollz::seal::DEFAULT_SEAL_CHUNK)]
    seal_chunk: u32,

    /// 封存压缩等级，默认 19（冷数据一次性付 CPU 换高比值）。
    #[arg(long, default_value_t = scrollz::seal::DEFAULT_SEAL_LEVEL)]
    level: i32,
}
...
fn run_seal(args: SealArgs) -> std::io::Result<()> {
    ...
    scrollz::core::validate_chunk_size(args.seal_chunk)?;
    ...
    let stats = scrollz::seal::seal_shadow_tree(&args.backing, args.seal_chunk, args.level)?;
...
fn run_ingest(args: IngestArgs) -> std::io::Result<()> {
    ...
    let s = scrollz::ingest::ingest_tree(
        &args.src,
        &args.backing,
        args.chunk_size,
        args.level,
...
fn run_compact(args: CompactArgs) -> std::io::Result<()> {
    if args.backend == Backend::Shadow {
        ...
        let s = scrollz::compact::compact_shadow_tree(&args.backing, DEFAULT_ZSTD_LEVEL)?;
        for (p, e) in &s.errors {
...
fn run_mount(args: MountArgs) -> std::io::Result<()> {
    ...
    let metrics = scrollz::core::metrics::Metrics::new();
...
fn serve_rw(
    ...
    cfg: &fuser::Config,
    metrics_file: Option<PathBuf>,
    metrics: Arc<scrollz::core::metrics::Metrics>,
) -> std::io::Result<()> {
...
mod cli_tests {
    ...
    fn parses_enable_guard_check_hidden_subcommand() {
        ...
        match cli.command {
            Some(Command::Enable(a)) => match a.action {
                Some(scrollz::enable::EnableAction::GuardCheck { name }) => {
    ...
    fn parses_enable_reconcile_undo_subcommand() {
        ...
        match cli.command {
            Some(Command::Enable(a)) => match a.action {
                Some(scrollz::enable::EnableAction::ReconcileUndo { name }) => {
                    assert_eq!(name, "-home-xp-src-foo");
                }
