...
impl Paths {
    ...
    /// backing 根目录 = scrollz_home/back（meta sidecar 与各 backing 的父目录）。
    pub fn back_root(&self) -> PathBuf {
        self.scrollz_home.join("back")
    }
    ...
    /// reconcile 快照暂存目录 = `scrollz_home/reconcile-stash/<name>/<ts>`。
    /// underlay 拍下的不可变快照落此处（合并输入与删前复核的唯一基准）；按 `ts` 分代便于审计/清理。
    pub fn reconcile_stash(&self, name: &str, ts: &str) -> PathBuf {
        self.scrollz_home.join("reconcile-stash").join(name).join(ts)
    }
    ...
    /// reconcile 隔离区 = `scrollz_home/reconcile-quarantine/<name>/<ts>`。
