#!/usr/bin/env bash
# zipfs-mount.sh — 幂等自挂载守护（后台运行 + PID + stale endpoint 清理）。
...
# 已是挂载点 → 幂等跳过（重复调用安全）。
if mountpoint -q "$MNT"; then echo "[zipfs] $MNT 已挂载，跳过"; exit 0; fi
...
for _ in $(seq 1 50); do mountpoint -q "$MNT" && { echo "[zipfs] 挂载就绪 $MNT (pid $(cat "$PID_FILE" 2>/dev/null))"; exit 0; }; sleep 0.1; done
echo "[zipfs] 挂载超时" >&2; exit 1
