#!/usr/bin/env bash
# mount-bs.sh — 用 scrollz 布局 S（shadow，影子树/每文件压缩包）读写挂载。
#
...
# 用法：scrollz --backend shadow --backing <dir> --mountpoint <mnt> --chunk-size 65536。
#
# 用法:
...
if [ ! -x "$BIN" ]; then
  cat >&2 <<EOF
[mount-bs] ERROR: 未找到 scrollz 二进制: $BIN
