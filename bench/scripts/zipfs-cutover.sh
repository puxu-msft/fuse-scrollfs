#!/usr/bin/env bash
# zipfs-cutover.sh — 可逆切换：把现有目录迁到 zipfs 压缩挂载，源备份保留，verify 通过才生效。
# 步骤：mv 源→源.zipfs-orig（备份）→ ingest --verify 灌入 backing → mount 到原路径。回滚见 zipfs-rollback.sh。
# 用法：zipfs-cutover.sh <目标目录> <backing> [chunk_size]
set -uo pipefail
...
"$BIN" --backend shadow --backing "$BACKING" --mountpoint "$TARGET" --chunk-size "$CHUNK" --pid-file "$TARGET.zipfs.pid" &
for _ in $(seq 1 50); do mountpoint -q "$TARGET" && { echo "[cutover] 已挂载，源备份在 $ORIG"; exit 0; }; sleep 0.1; done
echo "[cutover] 挂载超时，杀守护并 rollback" >&2; [ -f "$TARGET.zipfs.pid" ] && kill "$(cat "$TARGET.zipfs.pid")" 2>/dev/null; exit 1
