...
//! 免 `use zipfs::blockio::FaultIo` 找不到符号而红。运行：`cargo test --features fault-injection`。
...
fn active_seq_of(bytes: &[u8]) -> u64 {
    let a = zipfs::archive::parse_superblock(&bytes[SB_A_OFFSET as usize..]);
    let b = zipfs::archive::parse_superblock(&bytes[SB_B_OFFSET as usize..]);
    match (a, b) {
