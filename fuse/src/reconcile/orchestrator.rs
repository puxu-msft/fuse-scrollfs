...
/// 6. 快照：递归读每个 fall-through 文件落 stash 并 fsync，返回持锁 `Preconditions`。
...
/// 递归快照 `mp` 下所有 fall-through 文件到 `stash/underlay/<rel>` 并 fsync，返回内存快照。
///
/// 跳过 `is_harmless` 白名单项（与 guard 一致）。超 `MAX_MERGE_FILE_BYTES` 的文件不整体读入
/// `bytes`（留空，size/ino/mtime 仍记录，供 Task 7 降级 KeepBoth）；stash 侧仍原样拷贝以留证。
fn snapshot_underlay(mp: &Path, stash: &Path, ts_secs: String) -> io::Result<UnderlaySnapshot> {
    ...
    walk_snapshot(mp, mp, &underlay_stash, &mut entries)?;
    Ok(UnderlaySnapshot {
        ts: ts_secs,
        entries,
    })
...
fn walk_snapshot(
    ...
    for dent in std::fs::read_dir(dir)? {
        ...
        if file_type.is_dir() {
            walk_snapshot(root, &path, stash_underlay, entries)?;
            continue;
        }
        ...
        // 快照落盘：原样拷贝到 stash（含超限文件，留证），并 fsync 文件与父目录。
        ...
        // 内存快照：超限文件不整体读入内存（降级 KeepBoth 由 Task 7 消费）。
        let bytes = if size > MAX_MERGE_FILE_BYTES {
            Vec::new()
        } else {
            std::fs::read(&path)?
        };
