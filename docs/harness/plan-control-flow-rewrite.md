...
- **`record_invocation` 生产路径缺口顺手补上**：`budget.py` 现有 `record_invocation()` 已写好且有测试，但从未在生产路径被调用（`code-review-realmachine-fixes.md` 已指出）。它与 `BudgetTracker` 是两回事：`BudgetTracker` 管本轮扇出内部的并发原子预留（进程内存），`budget.record_invocation` 管持久化到 SQLite 的跨轮次审计记录。

  **⚠️ 调用位置已订正（Phase 5 合并态评审 `cfr-p5-merged-01`，实施前发现）**：本条**曾经**写着「在每次子调用真正返回后调用它」，并在下方 Task 6.1 具体化为「`_invoke_and_record(request)` 内部调用 `deps.invoke(request)` 并**紧接着**调用 `budget.record_invocation(...)`」。**这个形状是错的**——`invoke_fn` 由 `run_wave_scheduled` 在 **worker 线程**里调用，该闭包会**间接捕获**主线程的 SQLite connection，`run_wave_scheduled()` 会立即抛 `ProgrammingError: SQLite objects created in a thread can only be used in that same thread`。评审用真实 `sqlite3` connection 按该形状构造 `invoke_and_record` 复现了这一点。

  这与 cfr-03 是**同一条不变量**（worker 只返回纯数据、账本写入一律主线程串行），只是换了个入口悄悄破坏它。**正确形状**：`invoke_fn` 只执行模型调用并返回结果，`budget.record_invocation` 与 `ledger.record_attempt_*` 一样，由**主线程**在收集到 `AttemptRecord` 之后统一写入。实施时必须补一条**使用真实 `sqlite3` connection**（不是 fake）的集成测试，否则这个缺陷在纯 fake 测试下不可见。
...
5. `_run_round_body` 内，原有的"外层会话唯一职责是调 Workflow 再原样回显"这段旧代码整体替换为：构造 `agents = _load_agents(cfg.repo_root)` → 构造 `context = _build_request_context(cfg)`（cfr3-01）→ 构造 `deadline_monotonic`/`call_budget: BudgetTracker(total_usd=grant)` → 定义 `_invoke(request)`（**只**调用 `deps.invoke(request)` 并返回结果；**绝不**在其中写任何 SQLite——它跑在 worker 线程，见上方 `cfr-p5-merged-01` 的订正）→ 调用 `fanout.run_fanout(..., invoke_fn=_invoke, budget=call_budget, deadline_monotonic=deadline_monotonic, agents=agents, context=context, conn=deps.conn)` → 从返回的 `fanout_result["settlement"]` 取出 `total_turns`/`total_denials`/`exit_code`/`cost_known`/`total_cost_usd` 填入 `progress` 字典 → **在主线程**遍历 `settlement` 里的全部 attempts，逐条调用 `budget.record_invocation(...)` 写持久化审计（`cfr-p5-merged-01`：这一步**不能**放进 `invoke_fn`）。
