...
## 预算：上限是异常拦截器，不是成本目标（用户 2026-08-02 裁定）

**`HARNESS_ROUND_BUDGET_USD=100`、`HARNESS_DAILY_BUDGET_USD=2000`。**

用户的原话是「预算上限不是为了卡住正常运行，是为了拦截异常」。这条订正了此前把上限当成本目标的写法——**能在正常运行中被触发的上限是坏上限**：它产生的是假警报，还会逼人去调错的东西（缩小扇出、砍视角），而那些恰恰是用户已经明确否掉的方向（「成本问题不重要」）。

上限的正确语义：远高于正常值，**触发即意味着真的出了问题**——失控循环、重试风暴、模型陷入死循环。而不是「这一轮比平时贵了点」。

参考量级（真机实测，非外推）：单 finder $0.5987；contract probe $0.2617；旧架构完整轮 $5.45。

**因此不要再据「接近上限」去调整设计**。若某轮真的触发 $100，那是异常信号，应当去查为什么，而不是把上限调高或把扇出缩小。
