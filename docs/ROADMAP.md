...
> 日期：2026-06-28。
> 缺陷侧（已修 / 未做 + 判断依据）见 [06-defect-audit.md](./06-defect-audit.md)——两轮全面审查台账，本路线图的缺陷输入。
...
| **编码侧 zstd `--long`/更大窗口（长程匹配）** | 冗余主在**文件内长程**、重复点常 ≫1MiB 块距，放大 LDM 窗口是逼近 18–21x 单流上限**最便宜**的杠杆；ROI 高于去重 | 中 | ◐ **机制已落地**（提交 e9643b6；seal >8MiB 块自动开 LDM，windowLog=ceil(log2(chunk)) 硬 clamp ≤27，热路径不开、零回归，round-trip 逐字节测试兜底）。**但真实语料比值收益未实测**——合成样本仅证机制；调大默认 `DEFAULT_SEAL_CHUNK`（仍 8MiB）或对外宣称收益前，须 `ratio-bench` 真实 transcript 补测 LDM 开/关 × 大块 |
