...
> **✅ 全部完成（2026-06-30 后续会话）**：Bug A/B/C/D 均已 TDD 修复并提交（`fuse/` 11 commit `95775d3`..`e70bed9`，173 lib 测试 + 集成测试全绿）。neighbors 已迁到 systemd 模板 `scrollz@\x2d…neighbors.service`（enabled+active+watchdog），`diff -rq` vs rescue 金副本空、1529 文件逐字节一致。详见 memory `scrollz-corruption-fix-progress`。下文为当时的待办，已无效，仅留档。
...
- `pgrep -f "release/scrollz"` 会匹配命令自身,曾 `kill` 掉自己的 shell(exit 144)→ kill 守护用精确 PID 并排除 `$$`。
...
- 项目:`/home/xp/src/scrollz/fuse`(Rust 自研 FUSE 透明压缩文件系统);文档 `/home/xp/src/scrollz/docs/`。
- 它本是研究/评测项目,加了 `enable` 子命令后被用来挂载真实 `~/.claude/projects/*` 会话日志。
- **用户已拍板:绝不回撤数据,不讨论方向,唯一目标 = 把 scrollz 修对。** 不要再质疑该不该用它。
...
- 挂载点 `~/.claude/projects/-home-xp-src-neighbors`(systemd 托管的 scrollz 挂载,实时服务)
- backing `~/.claude-zip/back/-home-xp-src-neighbors`(146MB)
- orig 备份 `~/.claude/projects/-home-xp-src-neighbors.scrollz-orig`(1529 文件)
- rescue 硬链接 `~/scrollz-neighbors-rescue`(1529 文件)

neighbors 当前由**手动建的** unit `~/.config/systemd/user/scrollz-neighbors.service`(非模板)托管。
**不要对 neighbors 跑 `enable apply/restore`**(会再起第二个守护冲突);停起用
`systemctl --user stop/start scrollz-neighbors.service`。
...
- **Bug A** shadow backing 无并发锁 → open 时取 flock 排他锁。
  **锁文件必须放 backing 外 sibling `<backing>.scrollz.lock`**(放 backing 内会被 readdir
...
- **Bug C** 守护改 systemd per-project 模板 `scrollz@<name>.service` 托管(大重构,蓝图见计划)。
  要点:模板 `ExecStart=scrollz mount-managed --name %i`,新子命令读 sidecar meta 自拼参数;
  ...
  迁移前先杀旧裸 spawn 守护 + 清理旧 `scrollz-projects.service`/`scrollz-neighbors.service`。

**一条对 review 的再修订(务必改进计划)**:计划里"ingest/readdir 加 `.scrollz.*` 黑名单过滤"是**错的** ——
shadow backing 内文件名 = 用户原始文件名,按名字过滤会**误伤恰好同名的用户文件、违反零丢失**。
正解 = **控制文件一律放 backing 外**(backing 内纯用户数据,readdir 无脑透传,不加过滤);
可选 fail-loud(backing 内若发现 `.scrollz.*` 就警告而非隐藏)。
...
3. **`src/store/shadow.rs`**:
   ...
   - 模块级加 `fn lock_path_for(backing:&Path)->PathBuf` 返回 `<backing>.scrollz.lock` sibling
     (`backing.parent().join(format!("{name}.scrollz.lock"))`,用 `OsString::push` 避免 UTF-8 问题)。
   ...
   - mod tests(约 765 行,`use super::*`)加两个**英文名**测试:
     ...
     - `lock_file_lives_outside_backing`:open 后 `backing/.scrollz.lock` **不存在**、`<backing>.scrollz.lock` sibling **存在**。
...
- `classify` 真值表 + `ProjectStatus::Stopped` 在 `src/enable/model.rs`;`Paths`(meta_path=`back/<name>.scrollz.meta` sibling、backing、orig 布局)同文件。
...
- **过滤会误伤**:backing 内文件名=用户原始名,按 `.scrollz.*` 过滤分不清控制文件与同名用户文件。
- **提交标记独立**:meta 是"backing 可不可信"的外部裁决,放被裁决对象内部会破坏半灌检测的原子性。
→ 当前 `.scrollz.meta` 已在外(sibling),所以 readdir 透传是干净的;**用户早先看到的 `.scrollz.meta` 是旧损坏 backing(旧布局 meta 在内)的残留,已随恢复消失**。lock 必须同样在外。

## 8. 新会话第一步

1. `ls -la /home/xp/.claude/plans/cheeky-hatching-clock.md` + Read 它。
2. `cd /home/xp/src/scrollz/fuse && git status --short && cargo test --lib 2>&1 | tail -1`(确认干净基线 159 passed)。
