# scrollz 工程骨架现代化（PoC 转正遗留整改）

## Context

scrollz 起源是一次「btrfs+zstd vs FUSE 透明压缩」的横向评测实验（见 [docs/00-overview.md](../00-overview.md)）。FUSE 路线转正为整个产品后，顶层工程骨架仍带着 PoC 时代的形态，与内部已相当规整的模块化（`core/`、`store/`、`enable/`、`reconcile/`）不匹配。全面扫描发现四类骨架债务：
...
2. **`fuse/` 目录名是 PoC 化石** —— 它是当年「方案四 FUSE 路线」的目录，如今装的是整个产品（crate 名 `scrollz`），"fuse" 只是实现细节却占着顶层要名。
...
**目标结果**：仓库根一键 `cargo build` / `cargo test`（走 `default-members`，含产品 crate + 基准 crate）；统一 `Cargo.lock` 与 `target/`；产品 crate 落在语义准确的 `crates/scrollz/`；基准与 PoC 各归其位；两个巨型文件拆成内聚子模块。全程 `git mv` 保历史，分两阶段提交与审查。

> 归档 PoC（exp/）仍列为 workspace member，故 `cargo build/test --workspace`（显式全量）会额外编译它（含 `rusqlite { bundled }` 的 C 构建，首次较慢、之后缓存）。这是刻意选择：让 PoC 随 workspace 长期保持可编译（`keep-poc-in-project`），而日常一键用 `default-members` 跳过它。若将来 PoC 腐化拖累 `--workspace`，再降级为 `[workspace] exclude`（代价：根目录 `-p scrollz-microbench` 失效）。
...
scrollz/
  ...
  crates/
    scrollz/                          # ← 原 fuse/：产品 crate（lib + scrollz bin + mkfixture bin）
    scrollz-bench/                    # ← 新建：4 个基准 bin，依赖 scrollz lib
  exp/
    container-backend-selection/    # ← 原 microbench/：归档 PoC（default-members 排除）
  bench/                            # 不动（脚本/结果/数据集），仅修内部路径引用
  docs/                            # 不动，仅修 fuse/ → crates/scrollz/ 路径引用
```
...
[workspace]
resolver = "2"
members = ["crates/scrollz", "crates/scrollz-bench", "exp/container-backend-selection"]
default-members = ["crates/scrollz", "crates/scrollz-bench"]  # 默认 build 跳过归档 PoC

[profile.release]                   # 从两个子 crate 上提（workspace 只认根 profile）
lto = "thin"
codegen-units = 1
```
- `crates/scrollz/Cargo.toml`：删除其 `[profile.release]`（原 `fuse/Cargo.toml:106-108`）。
- `exp/container-backend-selection/Cargo.toml`：删除其 `[profile.release]`（opt-level=3 已是 release 默认；lto 交给根）。

### 1.2 迁移目录（git mv 保历史）

- `git mv fuse crates/scrollz`
- `git mv microbench exp/container-backend-selection`
- 新建 `crates/scrollz-bench/`（crate 名 `scrollz-bench`），把 4 个基准 bin 迁入：
  `git mv crates/scrollz/src/bin/{append-bench,ratio-bench,ldm-ratio,discovery-bench}.rs crates/scrollz-bench/src/bin/`
  —— **`mkfixture.rs` 留在产品 crate**（它是离线 fixture 生成工具，非基准）。
  - `crates/scrollz-bench/Cargo.toml` 依赖必须**完整声明 bin 直接 `use` 的 crate**（否则编译失败）：
    `scrollz = { path = "../scrollz" }`、`clap = { version = "4", features = ["derive"] }`（四个 bin 均 `use clap::Parser/ValueEnum`）、`libc = "0.2"`（`discovery-bench.rs:91-95` 直接用 `libc::posix_fadvise`）。传递依赖不可直接 `use`。
- 从 `crates/scrollz/Cargo.toml` 删除 4 个 `[[bin]]` 基准块（原 `fuse/Cargo.toml:27-51`），保留 `scrollz` 与 `mkfixture`；在 `crates/scrollz-bench/Cargo.toml` 声明对应 `[[bin]]`。
  - 源码零改动前提：4 个基准 bin 位于 `src/bin/`，只触及 `scrollz::` 公有 API，故 **bin 源文件无需改**；改动仅在上面的 Cargo.toml 依赖声明（下方验证兜底）。
- 删冗余：`git rm crates/scrollz/Cargo.lock exp/container-backend-selection/Cargo.lock`（workspace 生成唯一根 lock）；删两子 crate 的 `.gitignore`（内容仅 `/target`，根 `.gitignore:4-5` 的 `target/`+`**/target/` 已覆盖）。
...
- **bench 脚本（~15 处，最高风险）**：`bench/scripts/{scrollz-mount,scrollz-cutover,crash-test,crash-test-dm,crash-test-dm-logwrites,crash-test-container,mount-b0,mount-bs,mount-bv,ratio-matrix,ldm-ratio-matrix}.sh`。三种派生模式（`$REPO_DIR/fuse/...`、`FUSE_DIR=../../fuse`）统一改为指向根 `target/`；`cd fuse && cargo build` 提示改为 `cargo build --release -p scrollz`（基准 bin 改 `-p scrollz-bench --bin ratio-bench` 等）。
- **静态 systemd 单元** `bench/scripts/scrollz.service:15`：`ExecStart` 路径 `.../fuse/target/release/scrollz` → `.../target/release/scrollz`。
  （注：代码生成的 systemd 单元用 `current_exe()`，不受影响 —— 已核实。）
- **README**：根 `README.md`（目录树 + `cd fuse` + 11 行命令示例）、`crates/scrollz/README.md`（`../docs/` 相对链接因多嵌一层 → `../../docs/`）、`bench/README.md`（构建提示与产物路径）。
- **docs（仅活文档）**：`docs/02-layered-chunking.md:22` 的真实 MD 链接 `../fuse/src/...` → `../crates/scrollz/src/...`；`docs/{00-overview,01-scrollz-design,05-fault-injection-testing,07-hangfree-umount}.md` 的 `fuse/src/...` 散文引用与 `microbench/REPORT.md` 引用批量改写。`.claude/skills/scrollz-reconcile-ops/SKILL.md:27` 的 `cd .../fuse`。
  - **不改冻结历史**：`docs/archive/*.md`（已归档评审快照）与**已完成的** `docs/plan/*kickoff*.md`、`bench/results/**/REPORT.md` 里 `cd fuse && ...`、`git add fuse/src/...` 是当时真实执行过的命令，按项目文档规则属冻结快照，重写会篡改历史且对命令块做无差别替换有损坏风险。保持原样；如需可加一行「目录已迁移至 crates/scrollz/」批注，不动命令。
- **微移引用**：`README.md:21`、`docs/01-scrollz-design.md:144,286`（`microbench/REPORT.md` → `exp/container-backend-selection/REPORT.md`）；`exp/.../REPORT.md` 内 `../docs/` 链接因多嵌一层 → `../../docs/`。
- **本地配置** `.claude/settings.local.json:11-17`：按旧路径的许可 glob（`Read(.../fuse/**)`、`Bash(ls -1 fuse/)`、`Bash(./fuse/target/debug/scrollz *)`）随迁移改为 `crates/scrollz`（否则仅多几次授权提示，无功能损坏）。
...
2. `cargo test -p scrollz`（原 fuse 全部单测+集成测试）通过，数量与迁移前一致。
3. `cargo build -p scrollz-bench` 成功（4 基准 bin 编译）；`cargo build -p scrollz-microbench` 成功。
4. 抽查一个 bench 脚本 dry-run（如 `BIN=target/release/scrollz bench/scripts/mount-bs.sh` 的路径解析段）不再引用 `fuse/`。
...
6. `git log --follow crates/scrollz/src/main.rs` 显示历史连续（rename 保留）。

**阶段一提交**（语义单元分开）：`chore: 引入 Cargo workspace + 上提 profile`；`refactor: fuse/ → crates/scrollz/（git mv 保历史）`；`refactor: 基准 bin 拆出 crates/scrollz-bench`；`refactor: microbench → exp/container-backend-selection`；`docs: 路径引用随目录迁移更新`。
...
> 逐簇拆、逐簇 `cargo test -p scrollz` 绿，让编译器暴露遗漏的可见性。
...
- 先拆 pub 复用簇（`io.rs`/`delete_gate.rs`），每拆一簇即 `cargo test -p scrollz` 绿再继续。
...
1. 每拆一个子模块后 `cargo test -p scrollz` 全绿，测试数量与阶段一末尾**逐字一致**（`cargo test -p scrollz 2>&1 | grep 'test result'` 比对）。
...
- systemd 单元生成 `crates/scrollz/src/enable/autostart.rs` 用 `std::env::current_exe()` —— 迁移后自解析，无需改。
...
- **已采纳并修订**：C1（scrollz-bench 补 clap/libc 依赖）、C2（拆分需 `pub(crate)` 提升，附待提升清单）、I1（历史 `--follow` 只对 mod.rs 成立，验证如实改写）、I2（活文档清零 grep 覆盖 `fuse/src`）、I3（不改冻结 archive/ 与已完成 kickoff 历史命令）、I4（exp/ 作 member 的 trade-off 显式化，一键用 default-members）、M1（合并 lock 版本重解析提示）、M2（settings.local.json 旧路径许可）、M4（就近放测试规避私有可见性）。
- **未采纳，记录理由**：
  ...
  - M5（`scrollz-microbench` 包名不再暗示 `exp/` 位置）—— 纯外观，保持包名不变以减少 churn，验证一致用 `-p scrollz-microbench`。
