# scrollz 目标数据范围（Target Data Scope）

> 类型：范围决策（what）· 状态：**生效**（用户 2026-06-28 定，见 §4；决策台账 [ADR.md](./ADR.md) D7）。文档索引见 [README.md](./README.md)。

> 文档性质：**界定 scrollz 承载什么数据、分几期**。意图见 [00-overview.md](./00-overview.md)，实现见 [01-scrollz-design.md](./01-scrollz-design.md) 与 [02-layered-chunking.md](./02-layered-chunking.md)，路线见 [ROADMAP.md](./ROADMAP.md)。
> 实测日期：2026-06-28（`~/.claude` 全量扫描）。

## 1. 为什么要界定范围

`~/.claude` 总量约 **12 GB**，但「大」与「适合 scrollz」是两回事。scrollz 的研究价值在**灵活可定制**——per-file 透明、append 尾块缓冲、不可压缩 verbatim 跳过、冷文件 `seal` 重压、可选共享字典/去重。这些只在**单调 append-only、高冗余、可压缩**的数据上才兑现收益；对可重装代码、已压缩媒体则是纯开销。故必须**圈定范围**，而非「整个 `~/.claude` 一锅端」。
...
| ❌ 排除 | `.pack` / `.png` / `.pdf` | 已压缩，scrollz 不可压启发式应 **verbatim 原样存**，压了纯亏 |

## 4. 分层目标（核心决策）

| 期 | 目标 | 内容 | 为什么 |
|---|---|---|---|
| **Tier 1 · 首要** | **`projects/*.jsonl` + append 日志** | 8.0 GB jsonl + 两个 `.log` | scrollz 设计正为之而生：append 尾块缓冲、巨型 jsonl 的 footer 索引增量追加、冷会话 `seal` 重压、跨会话去重潜力。**研究价值最大、收益最实**。 |
...
## 5. 为什么这个范围放大 scrollz 的研究价值

把范围收到 **单调 append-only jsonl**，恰好让 scrollz 的「灵活可定制」相对内核 btrfs 形成**真实差异化**：
...
- **无 root / 无内核模块**：WSL 里 btrfs 要 root + modprobe + loop 镜像；scrollz 普通用户 FUSE 即可。

→ **结论**：纯压缩比内核 btrfs（force）能打平甚至更高，但在**这个范围**下，scrollz 靠**冷热分治（seal）+ 透明性 + 无 root**提供 btrfs 给不了的组合价值（去重为**待验证潜力**，非既得收益——见 §5 与 ROADMAP T3）。范围越聚焦 append-only jsonl，scrollz 越站得住。
