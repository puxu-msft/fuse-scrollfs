...
- **跨会话去重**：openvmm 那 10 个会话共享巨量相同前缀（系统提示 / CLAUDE.md / 重复文件读取）。内容寻址去重可能把 8 GB 压到**远超单纯 31x**——这是 btrfs extent 压缩**给不了**的（注：定长块去重实测 0% 命中，须 CDC，见 ROADMAP T3/G3）。
...
→ **结论**：纯压缩比内核 btrfs（force）能打平甚至更高，但在**这个范围**下，zipfs 靠去重 + 冷热分治 + 透明性提供 btrfs 给不了的组合价值。范围越聚焦 append-only jsonl，zipfs 越站得住。
