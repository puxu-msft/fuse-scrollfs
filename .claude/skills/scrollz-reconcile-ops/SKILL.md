---
name: scrollz-reconcile-ops
description: 操作 scrollz 的停用期回落写重合并——诊断 NEEDS-RECONCILE、跑 reconcile/reconcile-undo、处理 memory split-brain。当某 Claude project 停用期被直接写进裸挂载点、remount 被守卫拒、或需回退一次 reconcile 重选时用。
---

# scrollz enable reconcile / reconcile-undo 操作

> 项目原名 zipfs,2026-07-18 改名 scrollz(二进制/后缀/家目录随之改,见下)。
...
- `scrollz enable list` 里该项目标 **`NEEDS-RECONCILE`**（STOPPED 且 underlay 有回落写）。
...
- **零丢失**：全程 stash-backed。冷备份 golden 在 `~/.claude/projects/<name>.scrollz-orig`；reconcile 每代次 stash 在 `~/.local/claude-scrollz/reconcile-stash/<name>/<ts>/`。

## 标准流程

```bash
cd /home/xp/src/zipfs   # 仓库根仍为 zipfs（未改名）；或用已装好的 scrollz 二进制
# 1) 先 dry-run 看建议单（零改动，只读）
./target/release/scrollz enable reconcile -- <name> --dry-run
# 2) 逐条确认实跑。每条打印 推荐动作/置信度/理由：
#    a=accept（采纳推荐）  k=keep-both（都留、不动，延后）  s=skip（跳过）
#    高置信度条目【回车】即 accept；拿不准的选 s/k 延后（underlay 不动、可后续再来）
./target/release/scrollz enable reconcile -- <name>
# 3) underlay 清空后重挂 + 确认
./target/release/scrollz enable remount -- <name>
./target/release/scrollz enable list | grep <name>     # 期望 SCROLLZ（Active）
```
...
| **KeepSeparate**（疑 session-id 重用） | base 不动；重用那段会话隔离到 `~/.local/claude-scrollz/reconcile-quarantine/<name>/<ts>/<uuid>.jsonl`（保原名，可手动取回）。两段都留。 |
...
```bash
./target/release/scrollz enable reconcile-undo -- <name>   # 须未挂载
...
- **测试 backing 用 tempdir 子目录**（非 temp 根），否则 `.scrollz.lock` flock 跨测试碰撞 flaky。
